...
overall player performance = transpose(coach tactics) * (attack dimensions * skill dimensions * movement dimensions * power dimensions * mentality dimensions * defending dimensions * goalkeeping dimensions)
...
playeravailable <- filter(playeravailable, defending >= 75, physic >= 75) %>% subset(potential >= 75) %>% subset(age <=25)
...
playerwantedbrief
write.csv("draftIwantednomorethan25.csv", playerwantedbrief) # I will draft or trade in De Ligt and De Jong.
...
premierleaguedraw <- subset(demo, FTR == "D")
premierleaguetotaldrawcount <-nrow(premierleaguedraw)
premierleaguetotalwincount <- (premierleaguegamecount - premierleaguetotaldrawcount) / 2
premierleaguetotallosecount <- premierleaguetotalwincount
...
manchestercityfactor <- manchestercityoverall / manchestercityoverallp
chelseafactor <- chelseaoverall / chelseaoverallp
manchesterunitedfactor <- manchesterunitedoverall / manchesterunitedoverallp
...
liverpoolwinmanchestercityprediction <- liverpoolattackparameter * liverpoolfactor * manchestercitydefendparameter * manchestercityfactor * premierleagueaveragewin
liverpoolwinchelseaprediction <- liverpoolattackparameter * liverpoolfactor * chelseadefendparameter * chelseafactor * premierleagueaveragewin
liverpoolwinmanchesterunitedprediction <- liverpoolattackparameter * liverpoolfactor * manchesteruniteddefendparameter * manchesterunitedfactor * premierleagueaveragewin

liverpoolwinmanchestercityprediction
liverpoolwinchelseaprediction
liverpoolwinmanchesterunitedprediction

liverpoollosemanchestercityprediction <- liverpooldefendparameter * liverpoolfactor * manchestercityattackparameter * manchestercityfactor * premierleagueaveragelose
liverpoollosechelseaprediction <- liverpooldefendparameter * liverpoolfactor * chelseaattackparameter * chelseafactor * premierleagueaveragelose
liverpoollosemanchesterunitedprediction <- liverpooldefendparameter * liverpoolfactor * manchesterunitedattackparameter * manchesterunitedfactor * premierleagueaveragelose

liverpoollosemanchestercityprediction
liverpoollosechelseaprediction
liverpoollosemanchesterunitedprediction

# Frequencies of homewin and awaywin stat

library(tidyverse)

demo <- read.csv(file = "season-1819.csv", header = TRUE)
liverpoolvsmanchestercityhomewin <- demo %>% filter(HomeTeam == "Liverpool", FTR == "H") %>% filter(AwayTeam == "Man City")
liverpoolvsmanchestercityhomedraw <- demo %>% filter(HomeTeam == "Liverpool", FTR == "D") %>% filter(AwayTeam == "Man City")
liverpoolvsmanchestercityhomelose <- demo %>% filter(HomeTeam == "Liverpool", FTR == "A") %>% filter(AwayTeam == "Man City")
liverpoolvschelseahomewin <- demo %>% filter(HomeTeam == "Liverpool", FTR == "H") %>% filter(AwayTeam == "Chelsea")
liverpoolvschelseahomedraw <- demo %>% filter(HomeTeam == "Liverpool", FTR == "D") %>% filter(AwayTeam == "Chelsea")
liverpoolvschelseahomelose <- demo %>% filter(HomeTeam == "Liverpool", FTR == "A") %>% filter(AwayTeam == "Chelsea")
liverpoolvsmanchesterunitedhomewin <- demo %>% filter(HomeTeam == "Liverpool", FTR == "H") %>% filter(AwayTeam == "Man United")
liverpoolvsmanchesterunitedhomedraw <- demo %>% filter(HomeTeam == "Liverpool", FTR == "D") %>% filter(AwayTeam == "Man United")
liverpoolvsmanchesterunitedhomelose <- demo %>% filter(HomeTeam == "Liverpool", FTR == "A") %>% filter(AwayTeam == "Man United")

liverpoolvsmanchestercityhomewincount <- nrow(liverpoolvsmanchestercityhomewin)
liverpoolvsmanchestercityhomedrawcount <- nrow(liverpoolvsmanchestercityhomedraw)
liverpoolvsmanchestercityhomelosecount <- nrow(liverpoolvsmanchestercityhomelose)
liverpoolvschelseahomewincount <- nrow(liverpoolvschelseahomewin)
liverpoolvschelseahomedrawcount<- nrow(liverpoolvschelseahomedraw)
liverpoolvschelseahomelosecount <- nrow(liverpoolvschelseahomelose)
liverpoolvsmanchesterunitedhomewincount <- nrow(liverpoolvsmanchesterunitedhomewin)
liverpoolvsmanchesterunitedhomedrawcount <- nrow(liverpoolvsmanchesterunitedhomedraw)
liverpoolvsmanchesterunitedhomelosecount <- nrow(liverpoolvsmanchesterunitedhomelose)

liverpoolvsmanchestercityhomewincount
liverpoolvsmanchestercityhomedrawcount
liverpoolvsmanchestercityhomelosecount
liverpoolvschelseahomewincount
liverpoolvschelseahomedrawcount
liverpoolvschelseahomelosecount
liverpoolvsmanchesterunitedhomewincount
liverpoolvsmanchesterunitedhomedrawcount
liverpoolvsmanchesterunitedhomelosecount

liverpoolvsmanchestercityawaywin <- demo %>% filter(AwayTeam == "Liverpool", FTR == "A") %>% filter(HomeTeam == "Man City")
liverpoolvsmanchestercityawaydraw <- demo %>% filter(AwayTeam == "Liverpool", FTR == "D") %>% filter(HomeTeam == "Man City")
liverpoolvsmanchestercityawaylose <- demo %>% filter(AwayTeam == "Liverpool", FTR == "H") %>% filter(HomeTeam == "Man City") 
liverpoolvschelseaawaywin <- demo %>% filter(AwayTeam == "Liverpool", FTR == "H") %>% filter(HomeTeam == "Chelsea")
liverpoolvschelseaawaydraw <- demo %>% filter(AwayTeam == "Liverpool", FTR == "D") %>% filter(HomeTeam == "Chelsea")
liverpoolvschelseaawaylose <- demo %>% filter(AwayTeam == "Liverpool", FTR == "A") %>% filter(HomeTeam == "Chelsea")
liverpoolvsmanchesterunitedawaywin <- demo %>% filter(AwayTeam == "Liverpool", FTR == "A") %>% filter(HomeTeam == "Man United")
liverpoolvsmanchesterunitedawaydraw <- demo %>% filter(AwayTeam == "Liverpool", FTR == "D") %>% filter(HomeTeam == "Man United")
liverpoolvsmanchesterunitedawaylose <- demo %>% filter(AwayTeam == "Liverpool", FTR == "H") %>% filter(HomeTeam == "Man United")

liverpoolvsmanchestercityawaywincount <- nrow(liverpoolvsmanchestercityawaywin)
liverpoolvsmanchestercityawaydrawcount <- nrow(liverpoolvsmanchestercityawaydraw)
liverpoolvsmanchestercityawaylosecount <- nrow(liverpoolvsmanchestercityawaylose)
liverpoolvschelseaawaywincount <- nrow(liverpoolvschelseaawaywin)
liverpoolvschelseaawaydrawcount<- nrow(liverpoolvschelseaawaydraw)
liverpoolvschelseaawaylosecount <- nrow(liverpoolvschelseaawaylose)
liverpoolvsmanchesterunitedawaywincount <- nrow(liverpoolvsmanchesterunitedawaywin)
liverpoolvsmanchesterunitedawaydrawcount <- nrow(liverpoolvsmanchesterunitedawaydraw)
liverpoolvsmanchesterunitedawaylosecount <- nrow(liverpoolvsmanchesterunitedawaylose)

liverpoolvsmanchestercityawaywincount
liverpoolvsmanchestercityawaydrawcount
liverpoolvsmanchestercityawaylosecount
liverpoolvschelseaawaywincount
liverpoolvschelseaawaydrawcount
liverpoolvschelseaawaylosecount
liverpoolvsmanchesterunitedawaywincount
liverpoolvsmanchesterunitedawaydrawcount
liverpoolvsmanchesterunitedawaylosecount